<template>
  <div class="log_container">
    ...
    <div class="editbar">
      <div class="edit_btn">
        <el-button type="danger" size="mini" class="el-icon-delete" @click="deleteLogs()" round>
          移除
        </el-button>
        <el-button type="primary" size="mini" @click="exportLog()" round>
          导出数据
        </el-button>
      </div>
    ...
    >
      ...
      <el-table-column prop="logId" label="日志编号" width="50" align="center"> </el-table-column>
      <el-table-column prop="machineId" label="设备IP" width="120" align="center"> </el-table-column>
      <el-table-column prop="origin" label="错误来源" width="120" align="center"> </el-table-column>
      <el-table-column label="路由信息" width="300" align="center"> 
        <template scope="scope">
          <span style=" color:rgb(0, 153, 255)"> {{ scope.row.routeInfo }} </span>
        </template>
      </el-table-column>
      <el-table-column prop="level" label="等级" width="80" align="center"> </el-table-column>
      <el-table-column prop="message" label="基本信息" align="center"></el-table-column>
      <!-- <el-table-column prop="detail" label="详细信息" align="center"></el-table-column> -->
      <el-table-column label="时间" width="140" align="center">
        <template scope="scope">
          {{ $timeFormat.leaveTime(scope.row.date) }}
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    exportLog() {
      if (this.logIds.length == 0) {
        ...
        this.$api.nLog.exportLogs(this.logIds).then((res) => {
    ...
    async getNLogList() {
      await this.$api.nLog
        .getNLogList(this.queryForm.page, this.queryForm.row, this.queryForm.conditions, this.queryForm.level, this.queryForm.publicationDates)
        .then((res) => {
    ...
    //获取日志类型列表
    getNLogTypeList() {
      this.$api.nLog.getNLogTypeList().then((res) => {
        const { data, success, message } = res.data;
        if (!success) {
          console.log(message);
          return;
    ...
    deleteLogs() {
      if (this.logIds.length == 0) {
        ...
        this.$api.nLog.deleteLogs(this.logIds).then((res) => {
          let { success, message } = res.data;
          if (!success) {
            console.log(message);
